        ...
        <StaggerReveal className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {benefits.map((benefit) => (
            <StaggerItem key={benefit.title}>
              <div className="group relative bg-white border border-[#e2e8f0] rounded-2xl p-6 hover:shadow-[0_10px_40px_-10px_rgb(15_28_46/0.12)] hover:-translate-y-1 transition-all duration-300 h-full">
                {/* Icon */}
                <div
                ...
                {/* Stat */}
                <div className="mb-2">
                  <span className="text-3xl font-display font-bold text-[#0f1c2e]">
                    {benefit.stat}
                  </span>
                ...
                {/* Hover border accent */}
                <div className="absolute inset-x-0 bottom-0 h-0.5 bg-gradient-to-r from-[#f97316] to-[#ea6c00] rounded-b-2xl scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left" />
              </div>
            </StaggerItem>
          ))}
        </StaggerReveal>
