            ...
            <StaggerItem key={plan.id}>
              <div
                className={`relative rounded-2xl p-5 h-full flex flex-col transition-all duration-300 ${
                  plan.highlighted
                    ? "bg-gradient-to-br from-[#0f1c2e] to-[#1a2e48] text-white shadow-2xl scale-[1.02] border-0"
                    : "bg-white border-2 border-[#e2e8f0] hover:border-[#0f1c2e] hover:shadow-md"
                }`}
              >
                {plan.badge && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                    <span className="flex items-center gap-1 bg-[#f97316] text-white text-[10px] font-bold px-3 py-1 rounded-full shadow-md">
                ...
                <div className="mb-5">
                  <div className={`text-xs font-bold uppercase tracking-wider mb-1 ${plan.highlighted ? "text-[#f97316]" : "text-[#64748b]"}`}>
                    {plan.name}
                  </div>
                  <div className="flex items-end gap-1 mb-2">
                    <span className={`text-2xl font-display font-bold ${plan.highlighted ? "text-white" : "text-[#0f1c2e]"}`}>
                      {plan.price}
                    </span>
                    <span className={`text-sm mb-0.5 ${plan.highlighted ? "text-white/60" : "text-[#64748b]"}`}>
                      {plan.period}
                    </span>
                  </div>
                  <p className={`text-xs leading-relaxed ${plan.highlighted ? "text-white/70" : "text-[#64748b]"}`}>
                    {plan.description}
                  </p>
                </div>
                    ...
                    <li key={feature} className="flex items-start gap-2">
                      <Check
                        className={`w-3.5 h-3.5 mt-0.5 shrink-0 ${
                          plan.highlighted ? "text-[#f97316]" : "text-green-600"
                        }`}
                      />
                      <span className={`text-xs ${plan.highlighted ? "text-white/80" : "text-[#64748b]"}`}>
                        {feature}
                      </span>
                    </li>
                  ))}
                ...
                {plan.id === "enterprise" ? (
                  <Link
                    href="/contact"
                    className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-semibold transition-all duration-200 border-2 border-[#0f1c2e] text-[#0f1c2e] hover:bg-[#0f1c2e] hover:text-white`}
                  ...
                  <Link
                    ...
                    className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-semibold transition-all duration-200 ${
                      plan.highlighted
                        ...
                        : "border-2 border-[#0f1c2e] text-[#0f1c2e] hover:bg-[#0f1c2e] hover:text-white"
                    }`}
                  >
